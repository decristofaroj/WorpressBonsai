// Fix: Added ChatMessage type for conversational prompts.
...
// Fix: Added helper to format chat messages for the AI prompt.
const formatMessagesForPrompt = (messages: ChatMessage[]): string => {
    return messages.map(m => `${m.role === 'user' ? 'User' : 'AI'}: ${m.content}`).join('\n');
};

// Fix: Added a system prompt for the plugin builder's conversational AI.
...
export const getApplyFixPrompt = (plugins: Plugin[], issue: ReviewIssue): string => {
  // Fix: Corrected typo from formatPluginsFor to formatPluginsForPrompt and completed the template literal.
...
// Fix: Updated function to accept a message history for context instead of a single prompt.
...
// Fix: Updated function to accept message history and an optional file for context.
...
// Fix: Updated function to accept local analysis results for more intelligent packaging.
